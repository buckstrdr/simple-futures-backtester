"""Big Trend Double Check Strategy - Version 8 (DI Spread Filter).

BREAKTHROUGH: Adding DI Spread >= 20 filter to only trade strong directional moves.

DI Spread = |+DI - -DI| measures trend directionality:
- High DI spread = One directional indicator dominating = Clear trend
- Low DI spread = Both +DI and -DI similar = Choppy/ranging

Analysis showed DI Spread >= 20 improves:
- Win Rate: 36.1% → 37.7% (+1.6%)
- Return: -0.35% → +1.80% (+2.15% improvement!)
- Trades: 2911 → 1212 (filters out choppy periods)
...
class BigTrendScalperV8Strategy:
    """Big Trend scalping strategy with ADX 30+ and DI Spread >= 20 filters."""
    ...
    def __init__(
        ...
        di_spread_threshold: float = 20.0,  # KEY: New filter
