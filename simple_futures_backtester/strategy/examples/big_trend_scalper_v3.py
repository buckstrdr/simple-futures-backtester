"""Big Trend Double Check Strategy - Version 3 (Additional Filters).

Added filters to reduce trade count from 4458 to ~2225:
- Volume filter (above average volume)
- ATR filter (minimum volatility requirement)
- Stronger ADX threshold (25 instead of 20)

Strategy Logic:
- Entry: Momentum breakout (8+ points) + Close > SMA(5) + ADX > 25 + Volume > avg + ATR > min
...
class BigTrendScalperV3Strategy:
    """Big Trend scalping strategy with volume, ATR, and stronger ADX filters."""

    def __init__(
        self,
        # Entry parameters
        min_entry_momentum: float = 8.0,  # Points
        adx_period: int = 14,
        adx_threshold: float = 25.0,  # STRONGER: 25 instead of 20

        # NEW: Volume filter
        volume_ma_period: int = 20,
        min_volume_ratio: float = 1.2,  # Must be 20% above average

        # NEW: ATR filter (minimum volatility)
        min_atr_threshold: float = 15.0,  # Minimum ATR in points
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(start_idx, len(df)):
            ...
            if position == 0:
                # LONG ENTRY: All filters must pass
                long_entry = (
                    row['momentum'] >= self.min_entry_momentum and
                    row['close'] > row['sma_fast'] and
                    row['ADX'] > self.adx_threshold and  # STRONGER: 25 instead of 20
                    not pd.isna(row['ADX']) and
                    row['volume_ratio'] > self.min_volume_ratio and  # NEW: Volume filter
                    row['atr'] > self.min_atr_threshold  # NEW: ATR filter
                )
