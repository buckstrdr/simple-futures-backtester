"""Big Trend Double Check Strategy - Version 5 (Decoded Scalp Exit Logic).

BREAKTHROUGH: Decoded the actual scalp exit logic from 19-day data analysis.

Scalp Exit Characteristics (from original indicator):
- LONG EXIT: Strong bearish momentum bar (avg -6.3 points, close near low)
- SHORT EXIT: Strong bullish momentum bar (avg +7.0 points, close near high)
- Exit triggers when momentum reverses strongly opposite to position direction
- Close position in bar: Long exits ~27% (near low), Short exits ~74% (near high)

Strategy Logic:
- Entry: Momentum breakout + Close > SMA(5) for longs / < SMA(5) for shorts
- Exit: Strong opposite momentum (DECODED from actual indicator)
"""
...
class BigTrendScalperV5Strategy:
    """Big Trend scalping strategy with decoded scalp exit logic."""
    ...
    def __init__(
        ...
        scalp_exit_momentum_long: float = -5.0,  # Exit long when momentum < -5
        scalp_exit_close_position_long: float = 0.35,  # Exit if close < 35% of bar range

        scalp_exit_momentum_short: float = 5.0,  # Exit short when momentum > +5
        scalp_exit_close_position_short: float = 0.65,  # Exit if close > 65% of bar range
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """Generate entry and exit signals using decoded scalp exit logic."""
        ...
        start_idx = max(self.sma_fast_period, self.atr_period) + 1
